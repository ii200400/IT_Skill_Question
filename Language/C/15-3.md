...
```
// 포인터 상수이기 때문에 대입 자체가 불가능하다.
str1 = str2;
// 왼쪽이 문자열 리터럴이기 때문에 오류가 생긴다. 이는 선언과 동시에 초기화를 할 때에만 사용할 수 있는 방법이다.
str1 = "asdf";
```

포인터에서 배운 내용을 상기해보자 `int const *p`는 포인터 상수, `const int *p`는 상수 포인터이다.
...
- [15-4. 일로와봐, 문자열(string) - 도서 관리 프로젝트](./15-4.md)
