...
| method | description |
| --- | --- |
| init() | 서블릿이 메모리에 로드될 때 한 번 호출.<br>코드가 수정되면 새로운 코드 적용을 위해 기존 서블릿을 삭제 후 해당 메서드 호출 |
| doGet() | Get 방식으로 data 전송 시 호출 |
| doPost() | Post 방식으로 전송시 호출 |
| service() | 모든 요청이 service()를 통해서 doXX()메소드로 이동 |
| destroy() | 서블릿이 메모리에서 해제될 때 호출<br>코드가 수정되었을 때도 새로운 코드 적용을 위해 기존 서블릿 삭제를 위해 호출 |

### jsp (Java Server Pages)

웹 페이지를 구현하기 위해 Html을 작성할 때 자바를 활용하여 동적 페이지를 만들 수 있도록 하는 파일
