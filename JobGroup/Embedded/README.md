...
2. 무선 터치 센서를 만드는 도중 봐야했던 장비의 알 수 없는 스팩(specification)을 읽고 컨설턴트님의 도움을 받아 만들었던 것이 가장 힘들었다. LOLIN 사의 Wemos D1 mini WIFI 보드와 배터리 쉴드가 해당하는데 보드에 직접 납땜을 해야하는 구조여서 직접 납땜을 하였다. <br>
보드에 납땜하는 것은 처음이라 납땜으로 인한 과열로 쇼트가 자주 일어났고 기한을 못 지킬 뻔 하였다; 그래도 했던 일 중에서 가장 재미있던 것 같다. 가끔씩은 소프트웨어학과가 아니라 기계과를 갔어야 하는게 아닌가라는 생각도 한다.
