...
+ 용어 (깜찍하게 많으니 가볍게 읽고 넘어가고 아래의 내용에서 모르는 용어가 나올때 다시와서 읽자;)
  ...
  + MAC Address(맥 주소) : 네트워크 세그먼트의 데이터 링크 계층에서 통신을 위한 네트워크 인터페이스에 할당된 고유 식별자 이다.
...
에러검출, 재전송, 흐름제어 등을 다룬다. (에러를 수정하는 것은 전송계층이 한다.)
사용되는 장비로는 Bridge, Switch가 있다.
프로토콜은 HDLC, PPP, Frame-Relay, FDDI, ATM 등이 있다.

Data Link 계층은 두 가지 SubLayer로 나뉘는데, MAC과 LLC이다. MAC은 MAC주소를 이용해서 Data를 해당 장비로 보내는 것이고, LLC는 각 장비를 논리적으로 연결하고 유지하는 역할을 한다.

+ 네트워크 계층
데이터(패킷)의 경로를 찾아주고 가장 빠른 길로 전송하는 역할을 한다. (에러는 신경쓰지 않는다.)
사용되는 프로토콜로는 IP(인터넷 프로토콜), IPX, ICMP, ARP 등이 있다.
장비로는 라우터가 존재한다.

4. 전송계층
단말기 및 호스트 간의 데이터 전송 조율을 담당한다. 데이터의 용량, 속도, 목적지를 처리한다. 대표적으로 TCP와 UDP가 있다.

4-1. 전송계층 프로토콜 ( TCP / UDP )
TCP (Transmission Control Protocol, 전송제어 프로토콜) - 연결형 프로토콜
연결 설정 후 통신이 가능하다.
데이터를 재전송하여 신뢰성 있는 데이터 전송을 한다.
일대일 통신을 하며 데이터의 경계 구분이 없다.(바이트 스트림 서비스)
...
일대다 통신(멀티캐스팅이나 브로드캐스팅)을 하며 데이터 경계 구분이 있다.(데이터 그램 서비스)

5. 세션계층
통신을 하기 위해서는 세션(session)을 만들어야 하는데 이 작업이 여기서 처리된다. 
네트워크 연결 시작, 제어 및 운영, 연결 종료를 책임지며 데이터를 분리 및 유지하여 알맞은 Application으로 보내줄 수 있도록 한다.
OS(Operating System)가 이 계층에 속한다. 

6. 표현계층
데이터의 표현 형식(Format)을 정의한다. 네트워크와 응용프로그램 사이에서 데이터의 형식을 서로가 알아 볼 수 있는 모양으로 변환한다. 즉, 서로 다른 환경의 컴퓨터와 application들이 데이터를 이해할 수 있도록 도와준다.(번역기같은 느낌인 것 같다.)

JPEG, MPEG, QUICKTIME(동영상 포맷), MIDI(음악 포맷)등을 정의한다. data의 압축(compression), 암호화(Encryption)의 기능도 수행한다.
