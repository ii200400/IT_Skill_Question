...
가상 회선을 이용하는 연결 설정 후 통신이 가능하다.   
3-way handshaking과정을 통해 연결을 설정하고 4-way handshaking을 통해 해제한다.   
손실되거나 오류가 있는 데이터를 재전송하여 신뢰성 있는 데이터 전송을 하지만 속도가 UDP보다 느리다.

일대일 통신을 하며 (멀티캐스팅이나 브로드캐스팅을 지원x)   
데이터의 경계 구분이 없다. 바이트 스트림 서비스라고 불리며 데이터의 크기가 무제한이다.
...
일반적인 전화를 생각하면 이해가 쉽다.

재전송 요청으로 인하여 속도가 느리기 때문에 Streaming 서비스에 불리하다.

#### UDP (User Datagram Protocol, 사용자 데이터그램 프로토콜) - 비연결형 프로토콜

연결 설정 없이 통신이 가능하여 상대적으로 간단하게 구현 가능하다.   
흐름제어, 오류제어 또는 손상된 데이터 수신에 대한 재전송을 **하지 않는다**, 때문에 신뢰성은 낮지만 TCP보다 빠르다.   
단지, UDP헤더의 CheckSum 필드를 통해 최소한의 오류만 검출한다.

일대다 통신(멀티캐스팅이나 브로드캐스팅)이나 P2P에 자주 사용된다.   
데이터 경계 구분이 있는 데이터그램 단위로 전송되며 그 크기는 65535바이트로, 크기가 초과하면 잘라서 보낸다.

신뢰성보다 성능이 중요시 되는 실시간 서비스(streaming)에 자주 사용된다.
