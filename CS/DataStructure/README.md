...
아무리 cpu와 메모리가 강력해진다 하더라도 그 만큼 사람들이 더 어려운 문제를(대표적으로 빅데이터, AI) 컴퓨터를 활용하여 처리하기 때문이다.

Array와 List, 일부에서는 Queue 등.. 자료형처럼 사용하지만 엄밀하게 **자료구조**이다.   
기본적인 자료형(Int, Boolean, String 등)을 어떻게 관리할지, 이름 그대로 자료의 구조를 고안한 것으로   
데이터의 추가, 삭제, 정렬 등의 시간복잡도(Big-O)를 파악하는 것이 중요하다.
...
가장 기초적인 자료구조이다.\
선형 자료 구조는 이것으로 구현이 모두 가능하고 일부 비선형도 구현은 가능하다.
...
3. 값을 삭제하더라도 배열의 크기가 줄어들지 않아 메모리가 낭비된다.
5. 재할당을 하지 않는 이상 최대 원소의 갯수는 제한적이다.

#### 예시

순서대로 10, 20, 30, 40, 50을 Array에 넣은 후 40을 삭제하면   
10, 20, 30,  , 50 과 같이 40이 담겨있던 곳은 비어있게 된고 메모리 낭비가 일어난다.

</br>

### LinkedList (연결리스트)

일반적으로 리스트라고 하면 연결리스트를 의미하는 듯 하다.
...
1. 각 노드의 저장 위치는 관련된 노드의 포인터를 통해 알 수 있다.\
  즉, n 번째 원소는 n번의 포인트를 찾아가야 하므로 O(n)의 탐색시간이 걸린다.
...
3. 자체 간선(self-loops) : 자신에서 시작하여 자신으로 들어오는 간선 <img src="../image/1.1%20Graph1.PNG" width="5%" height="5%">
...
간선의 유무는 O(1)로 파악할 수 있지만, 특정 노드에서 인접한 모든 노드를 찾는 속도는 O(V)로 느린편이다.   
공간 복잡도는 V^2이다.   
...
배열(Array, HashTable, LinkedList 등)과 배열의 각 인덱스마다 존재하는 또 다른 배열을 이용하여 표현한다.   
그래프를 표현하는 일반적인 방법이다. 위에서 그래프의 기본이 되는 자료형이 LinkedList라고 했던 이유이다.

간선의 유무는 *관련된 노드의 차수만큼의 시간*으로 파악할 수 있고   
특정 노드에 인접한 모든 노드를 찾는 속도도 위와 같으며 이것은 행렬에 비해서 상대적으로 빠르다. 애초에 인접한 노드들만 저장을 했기 때문이다.   
...
+ 용어
  ...
  + leaf Node (=Terminal Node, 단말 노드) : 차수가 0인(자식이 없는) 노드, 단말(Terminal) 혹은 잎(Leaf)이라고 부른다.
...
perfect는 포화, complete는 완전, full은 전 으로 해석되어버려 개인적으로 햇갈린다.
