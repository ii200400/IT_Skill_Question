...
+ 구현 방법
  ...
  1. 인덱스 0은 비운 채로 1부터 노드의 데이터 값을 넣는다.
...
Black-Height 조건에 맞지않으면 rotation을 통해 height 를 조정하고 연속으로 Red 노드가 있다면 색을 조정한다.
...
## HashTable (해시 테이블)
hash는 기본적으로 배열을 사용하여 데이터를 저장한다.\
단, 보통의 인덱스가 아닌 Hash Function을 사용한 **고유의 인덱스**를 사용하여 데이터를 삽입/삭제할 때 추가적인 연산이 없도록 만들어졌다.
