...
2. 배열에 데이터를 더 이상 저장할 수 없을 때 배열의 크기 더 크게 재할당한다.
...
그래프가 가진 간선의 일부 혹은 전체를 이용하여 그 그래프의 모든 노드를 잇는 트리(비순환 유향 그래프)를 신장 트리(Spanning Tree)라고 한다.\
가중치 그래프에서 **사용한 간선의 가중치 합이 가장 적은 신장 트리**를 최소 신장 트리라고 한다.\
이 최소 신장 트리를 만드는 알고리즘은 대표적으로 2개가 있는데 아래와 같다.
