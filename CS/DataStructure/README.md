...
## HashTable (해시 테이블)
HashTable 또는 HashMap이라고 불리는 이것은 기본적으로 배열을 사용하여 데이터를 저장한다.\
단, 보통의 인덱스가 아닌 Hash Function을 통하여 만든 **고유의 인덱스**를 사용하여 데이터를 삽입/삭제할 때 추가적인 연산이 없도록 만들어졌다.

### Hash Function (해시 함수)
해시 함수는 임의의 길이를 갖는 임의의 데이터를 입력으로 받아서 고정된 길이의 데이터를 출력하는 함수를 말한다.\
이러한 해시 함수를 적용하여 나온 고정된 길이의 값을 해시값이라고 한다. 해시 코드(hash code), 해시섬(sum), 체크섬 등으로도 불린다.\
입력되는 데이터의 범위보다 출력되는 데이터의 범위가 좁다는 특징을 가지는데 이 때문에 다른 입력 데이터에서 같은 출력 데이터가 있을 수 있다.\
이러한 상황을 Collision(충돌)이라고 하며 이것을 잘 해결하는 것이 해시 함수의 핵심 중 하나이다.
