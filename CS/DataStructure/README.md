...
* [Graph](#graph)
  + [그래프 표현 방법](#그래프-표현-방법)
    - [1. 인접 행렬(adjacent matrix) - 행렬(2차원 배열)을 사용](#1-인접-행렬-adjacent-matrix----행렬-2차- 배열-을-사용)
    - [2. 인접 리스트(adjacent list) - 연결 리스트 사용](#2-인접-리스트-adjacent-list----연결-리스트-사용)
    - [3. 간선 리스트(Edge List) - 객체 리스트 사용](#3-간선-리스트-edge-list----객체-리스트-사용)
    - [그래프 선택](#그래프-선택)
* [Tree](#tree)
...
* [Hash (해시)](#hash--해시-)
  ...
  + [Collision Resolution](#collision-resolution)
    ...
    - [Open Address vs Separate Chaining(Linked List 기준)](#open-address-vs-separate-chaining-linked-list-기준-)
...
참고로 자바는 리스트가 인터페이스로 구현이 되어있고 ArrayList와 LinkedList를 통해서만 사용할 수 있기 때문에 추상자료형(abstract data type)이라고 말한다.
...
1. 자료의 논리적인 순서와 메모리 상의 물리적인 순서가 일치하지 않는다.   
   때문에 동적 배열처럼 물리적인 순서를 맞추기 위한 작업이 필요하지 않다.
2. 각 노드의 저장 위치는 관련된 노드의 포인터를 통해서만 알 수 있다.   
...
동적배열이 가장 뒤에 데이터를 추가할 때 O(1)이기는 하나, 내부 라이브러리에 의해 자주 크기가 바뀌면 생각보다는 더 오래 걸릴 수도 있다.   
하지만 보통은 크기를 증가시킬 때 기존 크기의 2배씩 늘려나가므로 크기를 넓히는 작업은 자주 불리지는 않는다.
...
4. 웹 브라우저의 이전 페이지와 다음 페이지 주소 url 저장에 사용된다.
...
3. front : enqueue를 할 때 큐에서 뺄 데이터의 위치, 혹은 마지막으로 삭제한 데이터 위치
4. rear/back : dequeue를 할 때 큐에 넣을 데이터의 위치, 혹은 큐에 저장된 마지막 데이터 위치
...
1. 인쇄기 대기열같은 일반적인 대기줄 / 대기열이나 버퍼(buffer)에 사용된다.
...
스택이나 큐와 비슷한 데이터 관리 방법 중 하나로 각 원소들이 가지고 있는 우선순위들을 기준으로 가장 큰 우선순위를 가지는 원소를 먼저 선택하는 특징을 가지는 **추상 자료구조**이다. 그래서 이름도 우선순위 큐이다.

우선순위 큐와 아래 서술될 [힙(heap)](https://github.com/ii200400/IT_Skill_Question/tree/master/CS/DataStructure#heap)과 같게 생각하는 경우도 있는데 힙은 우선순위 큐를 구현할 때 거의 항상 사용하는 자료구조일뿐 같은 것은 아니다. 마치 리스트나 스택을 구현할 때 배열과 링크드 리스트를 활용하는 것과 같다.

이 자료구조 또한 언어마다 특징이 다를 수 있기 때문에 본인이 사용하는 언어의 라이브러리가 어떻게 작동하는지 대략적으로 파악하는 것이 좋다.
...
7. **트리(Tree)**   
    한 노드를 가리키는 노드가 하나 이하인 비순환 그래프인 유향 그래프 
    혹은
    두 정점이 정확히 하나의 경로로 연결된 비순환 그래프인 무방향 그래프
...
#### 3. 간선 리스트(Edge List) - 객체 리스트 사용

간선의 정보를 객체로 만들어 리스트에 저장하는 방식이다.

대표적으로 최소 신장 트리 대표적인 알고리즘인 크루스칼, 프림 알고리즘에서 우선순위 큐와 함께 사용된다.   

#### 그래프 선택

간선의 정보를 저장하는 방식이나 메모리, 성능을 고려해서 결정한다.

일반적으로는 인접 행렬을 이용하는데   
희소 행렬에 가깝거나(정점이 1만개인데 간선이 10만개 정도인 경우) 인접 리스트를 사용하고   
크루스칼, 프림 알고리즘에서는 간선 리스트를 사용할 수도 있다고 한다.   
...
### 용어
  ...
  + Height (높이) : 루트 노드에서 특정 노드까지 사용되는 간선의 최소 수, 루트노드의 깊이는 0 이다.
  + 트리의 높이 : 트리의 모든 노드의 높이 중에 가장 큰 높이
  ...
  + 트리의 차수: 트리의 모든 노드의 차수 중에 가장 높은 차수
...
### 종류
  ...
  + Skewed Tree (편향 트리) : 노드가 한쪽으로만 추가된 듯 기울어진 트리
...
높이가 h인 이진트리가 최소로 가질 수 있는 노드 수는 h+1개, 최대로 가질 수 있는 노드의 수는 2^(h+1)-1개 이다.   
...
perfect는 포화, complete는 완전, full은 전 으로 해석되어버려 개인적으로 심각하게 햇갈린다;;
  ...
  + Complete Binary Tree (완전 이진 트리) : 마지막 높이를 제외한 모든 높이에서 노드가 순서대로 차있는 이진 트리   
...
보통 이진트리의 각 노드 번호는 루트노드를 1로 하고 높이가 낮은 노드부터, 왼쪽에 있는 노드부터 순차적으로 번호를 부여한다. 이런 방식으로 완전이진트리에 번호를 부여하면 아래와 같은 특징이 생긴다.

+ 노드 번호가 i인 노드의 부모 노드의 번호 : i/2
+ 노드 번호가 i인 노드의 왼쪽 자식의 노드의 번호 : 2*i
+ 노드 번호가 i인 노드의 오른쪽 자식의 노드의 번호 : 2*i+1
+ 레벨 n의 가장 왼쪽 노드 번호는 : 2^n

위의 특징으로 트리를 배열에 저장할 수 있다, 하지만 편향 이진트리가 되는 경우에는 메모리 공간 낭비가 심각하며 이를 피하기 위해 배열의 크기 변경을 하게된다면 노드를 삭제하거나 삽입하는 경우 배열의 크기 변경으로 인한 작업이 너무 번거로워 비효율적이게 된다.

배열을 이용하는 트리는 잘 고려해서 사용하자!
...
전위/중위 순회를 입력하면 후위순회를 출력하는 문제, 분할정복으로 구분되기도 한다.
...
이를 위하여 부모와 자식 간의 우선순위에 대한 조건이 있으며 이 조건을 충족시키기 위해서 **heap의 구조를 유지하는 것을 heapify**라고 한다.
...
해결 방법을 알아차리는 것이 어려운 문제이며 확인해보니 놀랍게도 플래티넘 레벨 문제이다..   
...
2. 루트 노드의 키가 왼쪽 서브 트리를 구성하는 노드들의 키보다 크다.
3. 루트 노드의 키가 오른쪽 서브 트리를 구성하는 노드들의 키보다 작다.
...
+ 탐색 연산에 O(lon n)(O(h)로도 표현)의 시간 복잡도가 필요하다.
...
키(key)를 알고있으면 대응하는 값(value)을 빠르게 찾을 수 있는 연관배열 구조를 가지는 자료구조이다.   

임의의 길이를 가지는 데이터인 키를 해시 함수를 통해 고정된 길이를 가지는 해시 값으로 바꿔주고 해시 값에 해당하는 버킷에 값을 저장하거나 탐색한다. 다른 키를 가지고 있어도 해시 함수를 통해 같은 해시 값이 나올 수 있는데 이를 충돌(Collision)이라고 부르며 이것을 처리하는 것이 해시 테이블을 중심이 된다.

단어의 정의가 정확히 성립되지 않은 것인지 Hash, Hashing, HashTable 또는 HashMap 모두 해시 함수를 이용하는 자료구조라는 느낌으로 언급된다. 

### 용어

  + Key (키) : 임의의 길이를 갖는 데이터이며 중복이 불가능하다.
  + Value (값) : 키와 매칭되어 저장되는 데이터를 의미한다.
  + 연관배열 구조 (Associative Array) : 키(key) 데이터와 값(value) 데이터가 1:1로 연관되어 있는 구조를 의미한다.
  + Hash Function (해시함수) : 키를 입력으로 받아서 해시값을 반환한다.   
    입력되는 데이터의 범위보다 출력되는 데이터의 범위가 좁다는 특징이 있으며 이 때문에 서로 다른 키에서 값은 해시값이 나올 수도 있다. 
  + Hash Value (해시값) : 키를 해시 함수에 넣은 후 나온 결과 값이다. 값들은 고정된 길이를 가지게 되며 저장소의 위치를 의미한다.   
  + Hash Table (=Hash Map, 해시 테이블) : 키와 1:1 대응 되는 해시 값들을 저장하는 테이블
    해시 코드(hash code), 해시 섬(hash sum), 체크 섬(check sum) 등으로도 불린다.
  + Bucket (=Slot, 버킷) : 값(value)이 저장되는 곳이다.
  + Collision (충돌) : 해시 함수에서 다른 키로 같은 해시 값이 생성되어 이미 사용하고 있는 저장소 위치에 접근하는 상황을 해시 충돌이라고 한다.
  
  <img src="../image/1.1%20Graph29.png" width="300">
  위키백과의 해시 테이블 예시 사진
...
즉, 해시 테이블은 충돌이 불가피하며 충돌을 적게하는 함수를 선택하면서도 충돌을 해결하는 방법을 아는 것이 중요하다.

이러한 해시함수는 유한한 범위의 출력하기 위해 간단하게 키를 임의의 값 N으로 나누기도 하며(H = k mod M) 문자열이나 수열을 처리하기 위해서 아래와 같은 해시 함수를 가지기도 한다.

<img src="https://user-images.githubusercontent.com/19484971/147951212-3ba9a31b-a585-4172-ade7-768a1e599233.png" width="100">   
해시함수 = 총합[키의 각 원소 * r^(키의 원소의 인덱스)] 나누기 M

여기서 H는 해시함수, L은 키의 크기, a는 키의 원소 중 하나, i는 인덱스, r은 임의의 수, M은 해시 값의 범위를 의미한다.   

자세한 의미는 [백준 15829](https://www.acmicpc.net/problem/15829)번의 문제를 정독하면 이해가 쉽다! 문제를 만들라고 했더니 해시를 풀이해 놓아서 꼭 읽어보기를 강력히 추천한다!   
또한 [영문 위키](https://en.wikipedia.org/wiki/Hash_table)도 자세하게 설명이 되어있으니 참고하자!

### Collision Resolution

충돌을 해결하기 위한 다양한 방법이 있지만 기본적으로 크게 두 가지 방식으로 충돌 해결법을 나눌 수 있다.

이 아래의 H는 해시함수 시간복잡도를 의미한다.   
...
+ 삽입/삭제/탐색 모두 최상의 경우 O(H), 최악의 경우 O(H+N)의 시간 복잡도를 가진다.
  + Worst Case라면 슬프게도 빈 공간을 찾지 못하고 원래자리로 돌아올 수 있다.   

이러한 방법은 대표적으로 3가지 종류가 있는데 다음과 같다.
...
처음 탐색한 위치를 f(k)이라고 하면 다음과 같이 탐색한다.   
 ...
 <img src="../image/1.1%20Graph27.png" width="350">
...
줄여서 체이닝(Chaining)이라고 하기도 하며 일반적으로 Open Address 보다 Separate Chaining이 더 빠르다. Open Address 의 경우 빈 버킷을 적어질수록 Worst Case 발생 빈도가 더 높아지기 때문이다.   
각 버킷을 리스트 형태로 구현하여 동일한 해쉬 값에 대해 여러 값을 한 버킷에 저장하여 충돌을 해결한다. 특징은 아래와 같다.

+ 한 버킷에 여러 데이터가 저장될 수 있어 한정된 해시 테이블에서 더 많은 데이터를 저장할 수 있다.
+ 연결 리스트의 특성을 이어받아 데이터의 크기가 동적이다.
+ 해시 함수를 선택하는 중요성이 상대적으로 낮다.
+ 한 버킷에만 데이터가 몰릴 수 있으며 이러한 경우 성능이 저하된다.
...
한 버킷에 데이터가 치중되는 최악의 경우 O(H+N)의 시간 복잡도를 가진다.

<img src="../image/1.1%20Graph28.png" width="550">

##### 2-2. 트리를 이용하는 방식 (Red-Black Tree)

위와 같은 방식으로 동작하지만 연결 리스트 대신에 자가균형 이진탐색트리인 Red-Black Tree를 사용하는 방법   

레드-블랙 트리의 특징을 이어받아 한 버킷에 데이터가 치중되는 어떠한 경우에도 O(H+logN)의 시간복잡도를 가진다.

한 버킷에 저장된 데이터 수가 많을수록 위의 Linked List 방법보다 효율적이다.   
반대로 데이터 수가 적다면 연결 리스트를 사용하는 것이 메모리를 절약할 수 있다.   
Red-Black Tree는 노드별로 색상을 지정하는 메모리도 추가로 필요해야하기 때문!
    
#### Open Address vs Separate Chaining(Linked List 기준)

1. 일단 두 방식 모두 Worst Case 에서 O(H+N)이다.
...
3. 버킷의 크기가 정적인 Open Address 방식에 비해 Separate Chaining 방식이 해시 테이블의 확장을 보다 늦출 수 있다.

연산과 데이터가 많은 오늘날, 데이터의 개수가 넘쳐나기 때문에 보통 체이닝을 사용하는 것으로 알고있다.

#### 기타

+ 보조 해시 함수 :  Separate Chaining 방식을 사용할 때, 해시값을 변형하여 해시 충돌 가능성을 줄이는 해시 함수를 보조하는 함수이다.
