...
+ 구현 방법
1. 시작 노드부터 최단거리를 저장할 배열을 만들고 배열에 출발 노드에는 0을 출발점을 제외한 다른 노드들에는 매우 큰 값을 넣는다.
...
+ 특징
1. 인접행렬로 구현시 시간 복잡도 O(V^2)를 가지고 (최대)힙(Heap)을 활용하면 O((V+E)logV)의 시간 복잡도를 가진다.
...
2. 시작 노드에서 목표 노드까지의 최단거리를 구하는 과정에서 시작 노드와 그 외의 노드의 최단거리까지 구해버린다.

+ 예시 사진

<img src="../image/1.1%20Graph14.gif" width="40%" height="40%">
~~마지막에 노드 4의 값(20)과 '5-4 간선+노드 5의 값'(26)을 비교해야 할 것 같은데 생략해 버린다;;~~
...
**음의 가중치가 있는 그래프에서도 사용이 가능**하다.

에지 k개를 이용해서 도달할 수 있는 노드들을 모두 구했다면, 어떤 노드 i가 (시작 노드부터 노드 i까지 에지 k개를 사용하여 도착할 수 있는  최단 거리) + (노드 i에서 노드 j를 잇는 에지의 가중치) < (시작 노드부터 노드 j까지 에지 k개를 사용하여 도착할 수 있는 최단 거리) 라면, 에지 k+1개를 사용할 수 있는 경우 시작 노드부터 노드 j까지의 
