...
루트 노드에서 시작하여 갈 수 있는 최대한 깊은 곳까지 탐색해나가는 탐색방법, 깊이 우선 탐색으로 불리며 stack이나 재귀함수를 사용하여 구현한다.   
...
루트 노드에서 시작하여 현재 노드에서 접근할 수 있는 자식 노드들을 우선으로 방문한 뒤에 방문했던 노드들의 자식 노드를 다시 차례로 방문하는 방식, 너비우선탐색이라고 불리며 큐로 구현한다.
