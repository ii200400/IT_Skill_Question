...
각 쓰레드는 독립적인 작업 수행을 위해 스레드 ID, 프로그램 카운터, 레지스터 집합, 그리고 스택 영역을 가지며   
...
+ Mutual Exclusion(상호 배제)   
  프로세스 P1 이 임계영역에서 실행중이라면, 다른 프로세스들은 그들이 가진 임계영역에서 실행될 수 없다.
+ Progress(진행)   
...
이러한 조건을 만족하는 데이터 동기화 방법으로 대표적으로 2가지가 있다.

#### LOCK
하드웨어 기반 해결책으로 동시에 공유 자원에 접근하는 것을 막기 위해 LOCK을 사용하는 방법
임계영역에 진입하는 프로세스는 Lock 을 획득하고 Critical Section 을 빠져나올 때, Lock 을 해제으로써 동시에 접근이 되지 않도록 한다.
