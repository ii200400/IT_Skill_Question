...
+ 용어
  + 페이지(Page) : 논리메모리(프로세스가 가진 메모리)를 같은 크기의 블록으로 나눈 것
...
페이지와 프레임을 대응시키기 위해 page mapping 과정이 필요하며 paging table을 통해서 mapping이 된다.
...
페이지 단위를 작게하면 내부 단편화 문제도 해결할 수가 있으나, 대신 page mapping 과정이 많아지므로 오히려 효율이 떨어질 수 있다.
반대로 단위를 크게 하면 내부 단편화 문제가 심화되고 page mapping 과정이 적어지므로 적절한 단위를 설정해야 한다.
