...
프로세스는 보통 5가지의 상태를 가진다. (메모리가 부족할 경우 추가로 2가지의 상태를 더 가지게 된다.)

<img scr="../image/1.3 Process2.png" width="40%" height="40%">
  ...
  + 보류 대기 : 대기 상태에서 프로세스가 메모리를 잃은 상태, 입출력이나 이벤트가 끝나면 보류 준비상태로 바뀐다.
  + 보류 준비 : 대기 이외의 상태에서 프로세스가 메모리를 잃은 상태, 메모리에 여유가 다시 생기면 다시 메모리를 할당 받는다.
