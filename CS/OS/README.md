...
동기화가 잘 이루어지지 않는다면 같아야하는 변수 값이 프로세스(쓰레드)마다 다른 상황이 일어난다.
...
이러한 조건을 만족하는 데이터 동기화 방법으로 대표적으로 뮤텍스와 세마포어가 있다.   
*동기화 방법으로 모니터(Monitor)도 있지만 생략하겠다.*

#### 뮤텍스(Mutual Exclusion, Mutex 또는 Lock, 락)

뮤텍스는 상호배재라는 뜻으로, 동기화 기법 중 하나이다.
동시에 공유 자원에 접근하는 것을 막기 위해 lock을 활용하는 방법으로 하나의 프로세스만이 임계구역에 접근하게 된다.
임계영역에 진입하는 프로세스는 lock을 획득하고 임계영역을 빠져나올 때, lock을 해제(unlock)으로써 동시에 접근이 되지 않도록 한다.

#### 세마포어(Semaphores)

wait과 signal을 사용하여 복수의 프로세스(쓰레드)가 동시에 임계영역으로 접근하는 동기화 기법이다.

1. wait을 먼저 호출하여 임계 영역에 들어갈 수 있는지 확인한다.    
   아래는 바쁜 대기를 활용하여 만든 wait 함수이다.
   + 바쁜 대기(busy waiting 또는 spinning)   
...
운영체제 혹은 소프트웨어의 잘못된 자원 관리로 인하여 둘 이상의 프로세스가 함께 멈추어 버리는 현상   
세마포어에서 둘 이상의 프로세스가 서로의 자원이 필요하여 wait 할 때, 발생한다.
...
+ 뮤텍스는 lock이 되어있다/아니다 의 두 가지 상태만 표시하면 되기 때문에 바이너리(Boolean) 변수이지만,
  세마포어는 정수 변수(S)를 가진다.
+ 뮤텍스는 lock을 한 프로세스만이 unlock이 가능하다. 
  세마포어가 가지고 있는 변수(S)는 자원이 필요한 어떤 프로세스라도 변경 가능하지만 한 시점에 하나의 프로세스만 변경이 가능하다.
