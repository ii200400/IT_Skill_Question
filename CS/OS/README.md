...
프로세스가 각자 메모리를 가지는 것처럼 운영체제도 프로세스들을 관리하기 위하여 PCB라는 별도의 자료구조를 만들어 가지고 있다.

운영체제는 프로세스 생성과 동시에 해당 프로세스와 대응하는 고유한 PCB를 생성한다.   
프로세스가 작업을 중단(혹은 보류)해야 한다면 진행 중인 작업을 PCB에 저장하고 가지고 있던 자원을 반환시킨다.   
이후 프로세스가 다시 작업을 시작할 때, PCB에 저장되어있던 내용을 불러와 다시 작업을 시작할 수 있도록 한다.   
프로세스가 종료되면 프로세스가 메모리에서 삭제되고 PCB 또한 폐기된다.
