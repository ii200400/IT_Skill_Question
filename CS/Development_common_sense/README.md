...
디자인 패턴 중 MVC 패턴은 다양한 플렛폼에서 쓰이는 기본적인 패턴 중 하나이기 때문에 해당 카테고리에 넣었다.

이름 그대로 Model, View, Controller 세 가지 형태로 역할을 나누어 개발하는 방법론이다.

### 구성

+ Model : 데이터를 저장하고 관리하는 역할
...
알고보니 같은 MVC 패턴이 맞고 입력이 뷰를 통하지 않는 프로그램들(아마도 서버..?)이 있는 것 같다.   
...
+ 컨트롤러는 뷰와 모델사이에서 중개인 역할을 한다.   
  +  컨트롤러에게 너무 많은 작업이 몰려 프로그램이 커질수록 코드 분석이나 테스트가 어려워질 수 있다. (이미지 참고)
+ 뷰는 어떤 컨트롤러는 작업을 진행시키는지 알 수 없다.
